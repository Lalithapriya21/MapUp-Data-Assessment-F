...
def calculate_distance_matrix(dataset):
    ...
    for i in range(len(df_distance)-2):
        for j in range(len(df_distance)-2-i):
             df_distance.iloc[j+2+i]=df_distance.iloc[j+1+i]+df_distance.iloc[j+2+i,j+1+i]
...
#Creat a Dataframe with the Day combination
...
result_df=pd.merge(result_df,day_combonations_df_weekdays,on=['id_start','id_end'])
weekend_comb=pd.merge(weekend_time_ranges_df,day_combinations_df_weekends,on=['id_start','id_end'])
rresult_df1=pd.merge(result_df[['id_start','id_end','distance','moto','car','rv','bus','truck']],weekend_comb,on=['id_start','id_end'])
